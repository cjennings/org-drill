...
*** DONE [#B] Fix race condition with timer cleanup (Lines 1662-1665, 1768)
CLOSED: [2025-11-13]
:PROPERTIES:
...
*Resolution:* Wrapped recursive-edit in org-drill-presentation-prompt with unwind-protect to ensure timer is always cancelled even on abnormal exit. Commit 2fd6b66.


*** DONE [#B] Fix marker leak in error cases (Lines 2687-2700)
CLOSED: [2025-11-13]
:PROPERTIES:
...
*Resolution:* Moved org-drill-free-markers call outside conditional block to always free done-entries markers even on error or suspension. Commit 2fd6b66.

** SEVERITY C - MEDIUM PRIORITY ISSUES

*** DONE [#C] Fix inconsistent error handling for empty property blocks (Line 2138)
CLOSED: [2025-11-13]
:PROPERTIES:
...
*Resolution:* Replaced unsafe fallback with proper positioning logic using org-end-of-meta-data when property block is missing. Commit c524860.

*** DONE [#C] Remove or implement unused variable in commented code (Line 1472)
CLOSED: [2025-11-13]
:PROPERTIES:
...
*Resolution:* Removed obsolete commented code. The failures variable is already obtained from org-drill-entry-failure-count, making the commented code redundant. Commit b9b34df.

*** DONE [#C] Fix buffer corruption in org-drill-merge-buffers (Lines 3221-3223)
CLOSED: [2025-11-13]
:PROPERTIES:
...
*Resolution:* Replaced switch-to-buffer with with-current-buffer to prevent window state corruption and allow batch mode operation. Commit 1a759fe.

*** DONE [#C] Extract hardcoded magic number for lapsed days (Lines 2709-2710, 2726)
CLOSED: [2025-11-13]
:PROPERTIES:
...
*Resolution:* Created org-drill-lapse-threshold-days defcustom (default 90) to replace hardcoded values. Now user-customizable and maintained in one place. Commit 679632c.

*** DONE [#C] Fix incorrect property name in org-drill-show-answer-verb-conjugation (Line 3228)
CLOSED: [2025-11-13]
:PROPERTIES:
...
*Resolution:* Changed LAST_QUALITY to DRILL_LAST_QUALITY to match standard naming convention. Commit 4ce9d67.

*** DONE [#C] Fix property name typo (Line 3231)
CLOSED: [2025-11-13]
:PROPERTIES:
...
*Resolution:* Changed LAST_REVIEWED to DRILL_LAST_REVIEWED to match standard naming convention. Commit 4ce9d67.

*** DONE [#C] Add error handling in org-drill-entry-days-since-creation (Line 2738)
CLOSED: [2025-11-13]
:PROPERTIES:
...
*Resolution:* Wrapped org-time-stamp-to-now in condition-case to gracefully handle malformed timestamps by returning nil. Commit 37f8bc7.
...
- Calculate success rate: (cards rated ≥3) / (total cards reviewed)
...
*** DONE [#A] Fix potential division by zero in org-drill-entry-overdue-p
CLOSED: [2025-11-13 Wed]
:PROPERTIES:
:LOCATION: org-drill.el:935
:SEVERITY: B
:END:
...
*Fix Applied:*
- Added check =(> last-interval 0)= before the division operation
- Prevents division by zero for new items or items after failures

*Resolution:* Fixed in commit becb992

*** DONE [#A] Fix infinite loop in marker selection
CLOSED: [2025-11-13 Wed]
:PROPERTIES:
:LOCATION: org-drill.el:2619-2671
:SEVERITY: B
:END:
...
*Fix Applied:*
- Added safety counter with max-attempts limit of 1000
- Loop now exits if max attempts reached
- Returns nil if no valid entry found after exhausting attempts
- Increments attempts counter on each iteration

*Resolution:* Fixed in commit fe73c80

*** DONE [#B] Fix docstring typo: "dasy" should be "days"
CLOSED: [2025-11-13 Wed]
:PROPERTIES:
:LOCATION: org-drill.el:925
:SEVERITY: B
:END:
...
*Fix Applied:*
- Changed "dasy" to "days" in docstring
- Also improved docstring style from "Returns true" to "Return non-nil" (Emacs convention)

*Resolution:* Fixed in commit 281cb65
